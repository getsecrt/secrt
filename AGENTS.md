...
7. Tag the changed component(s) with the YubiKey-backed signer: `git-tag-sk -m "secrt-server vX.Y.Z" server/vX.Y.Z` (or `secrt-cli vX.Y.Z` for the CLI). Tag only what has a CHANGELOG entry — `extract-changelog-entry.sh` will fail the workflow on missing entries.

   **Tag message convention:** keep it to the short component identifier — `secrt-server vX.Y.Z` or `secrt-cli vX.Y.Z` — matching v0.17.7 and onward. The release body is generated from the CHANGELOG, so the tag message should not duplicate release notes. Verbose descriptive forms (e.g. `"fix QR scanner CSP worker-src"`) are inconsistent with the established pattern.

   **Agent note:** `git-tag-sk` is a YubiKey-backed zsh function in `~/.zshrc` that requires PIN + touch on a real TTY — agents cannot run it on the user's behalf. Prepare the bump commit and stop; the user creates and pushes the tag.
