...
- `secrt pair [<code-or-url>]` (API-key authenticated, share the account key between devices)
- `secrt sync <url>` (legacy, API-key authenticated, import the account key from a one-time link)
...
1. CLI selects plaintext input source:
   ...
   - When reading from stdin with a TTY attached:
     ...
     - Multi-line mode (e.g., `--multi-line` / `-m`): implementations SHOULD support reading until EOF (Ctrl+D on Unix; Ctrl+Z on Windows), preserving exact bytes. This is intended for pasting multi-line content like SSH keys, certificates, or config snippets. Implementations SHOULD show the platform-appropriate EOF gesture in any interactive prompt and `--help` output.
...
- If the AMK is unavailable and `enc_meta` is present, the Notes column displays `(encrypted)` in WARN color. A hint is printed to stderr: "Pair this device (`secrt pair`) to view your encrypted notes."
...
Shares the account key (AMK) between two devices signed in to the same
account, so paired devices can read and write encrypted notes. Auto-detects
mode based on local account-key presence:

- **Send mode** (this device already has the account key): prompt for an
  8-character pair code shown on the other device, or accept it via a
  positional `<code-or-url>` argument. Lenient input — accept bare
  `XXXX-XXXX` / `XXXXXXXX` (any case, whitespace stripped) **and** full
  `/pair?code=…` URLs. Canonicalize to `XXXX-XXXX` before sending.
- **Receive mode** (this device does NOT have the account key): generate
  an ephemeral P-256 ECDH keypair, register the public key with
  `POST /api/v1/auth/pair/start`, render the user code + a QR encoding
  the full pair URL, poll `POST /api/v1/auth/pair/poll` until terminal.

Usage:

```bash
secrt pair [<code-or-url>] [--api-key <key>] [--base-url <url>] [--json] [--silent]
```

Behavior:

1. Resolve the local account-key state via `GET /api/v1/amk/wrapper` and
   the configured API key. Choose mode strictly:
   - **Present + no positional → Send (prompt for code).**
   - **Present + positional → Send (use given code, skip prompt).**
   - **Missing + no positional → Receive.**
   - **Missing + positional → exit 1**, "this device does not have the
     account key; run `secrt pair` (with no code) to receive it…".
   - **NoApiKey / InvalidApiKey → exit 1**, point at `secrt auth login`.
   - **CorruptWrapper → exit 1**, suggest re-link via `secrt auth login`.
   - **NetworkError → exit 1**, transient.
2. Send mode wire crypto: generate an ephemeral P-256 ECDH keypair,
   derive a shared secret with the displayer's public key (looked up via
   `GET /api/v1/auth/pair/challenge?user_code=…`), HKDF-SHA256 to a
   32-byte transfer key (info `"secrt-amk-transfer-v1"`, empty salt),
   AES-256-GCM encrypt the AMK with AAD `"secrt-amk-transfer-v1"` and a
   random 12-byte nonce, and `POST /api/v1/auth/pair/approve`.
3. Receive mode wire crypto: same primitives in reverse — on
   `pair_poll → approved`, decrypt with the local private key and
   persist via the same wrap+upload path used by `sync` and `auth login`.
4. Cross-instance URL guard: when the positional argument is a URL,
   apply the same leak guard `sync`/`burn`/`info` use. Cross-instance
   URL → **exit 2** (matches existing guards' exit code).
5. Parser-rejection (invalid code shape, malformed URL) → **exit 2**.
   All other operational failures (network, 403, 429, expired code,
   decrypt failure, AMK persistence failure) → **exit 1**.

The user-code format is exactly 8 characters from the alphabet
`ABCDEFGHJKLMNPQRSTUVWXYZ23456789` (32 chars; excludes `0`, `1`, `I`,
`O`). Server is case-sensitive on the wire; the CLI uppercases input.

Output:

- Send mode default: print `✓ Account key sent.` on stderr.
- Receive mode default: print the code + the full pair URL + QR (encoding
  the full pair URL) on stderr; print countdown progress on stderr; on
  success print `✓ Account key received.` on stderr.
- `--json` mode: progress on stderr; final `{"status":"sent"}` or
  `{"status":"received"}` on stdout. Nothing else on stdout.
- `--silent`: suppress countdown / "Encrypting…" lines, but **the user
  code and QR in Receive mode are still printed** — they are the point of
  the command.

## `sync` (legacy)

Imports the account key (AMK) from a sync link generated by the web UI.
**Legacy** — kept for headless / scripted setups where no human is at the
terminal to approve a pair code. For everyday device setup, use
`secrt pair` instead. The sync URL is not surfaced in the web UI by
default.
...
- Default: print a success message on stderr (`✓ Account key synced successfully`).
...
6. If the server is reachable, check account key status via `GET /api/v1/amk/wrapper`:
   - If an AMK wrapper exists: print `Account key: synced` in SUCCESS color.
   - If no wrapper: print `Account key: not synced` in WARN color with a hint pointing at `secrt pair`.
