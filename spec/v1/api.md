...
- `HKDF_INFO_AMK_WRAP = "secrt-amk-wrap-v1"`
- `AMK_COMMIT_DOMAIN_TAG = "secrt-amk-commit-v1"`

##### AAD construction

The AAD binds each wrapper to a specific `(user, key_prefix, version)` tuple, preventing cross-replay across accounts, API keys, or future versions.

```
AAD = info || user_id_uuid || u16be(len(key_prefix)) || key_prefix || u16be(version)
```

Where:

- `info` MUST equal `"secrt-amk-wrap-v1"` (UTF-8, 17 bytes).
...
- `key_prefix` MUST be the UTF-8 bytes of the API-key prefix (≥6 chars from `[A-Za-z0-9_-]`).
- `version` MUST be big-endian u16. In v1, `version = 1`.

**Convention:** fixed-size protocol primitives (UUIDs, the version field) are included verbatim with no length prefix. Variable-length fields (e.g., `key_prefix`) are prefixed with a u16 big-endian byte length. This rule applies to every AMK wrap path, including the future PRF wrap path.
...
Other transports (sync link, ECDH transfer, future PRF) derive a 32-byte wrap key via their own mechanisms and then use the same AAD, cipher, nonce, and byte layout defined here. See §AMK transport.
...
##### Transport D: PRF wrap (future)

A fourth transport is planned: deriving the wrap key from a WebAuthn PRF extension output, so a new device with access to a synced passkey can derive the same wrap key with no bearer-token exchange. The in-progress design lives at `crates/secrt-server/docs/prf-amk-wrapping.md`. The wrap format defined in §AMK wrapping is designed to accommodate this transport unchanged; only the IKM source and info string differ.
