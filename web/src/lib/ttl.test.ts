...
describe('TTL constants', () => {
  ...
  it('presets are in ascending order', () => {
    for (let i = 1; i < TTL_PRESETS.length; i++) {
      expect(TTL_PRESETS[i].seconds).toBeGreaterThan(
        TTL_PRESETS[i - 1].seconds,
      );
    }
  });
});
...
describe('CLI spec vectors - invalid TTL values', () => {
  const invalidWithSeconds = [
    ...
    { input: '-1', seconds: -1, reason: 'negative value' },
    {
      input: '31536001',
      seconds: 31_536_001,
      reason: 'exceeds maximum TTL (1 year + 1 second)',
    },
    {
      input: '366d',
      seconds: 366 * 86_400,
      reason: 'exceeds maximum TTL (366 days)',
    },
    {
      input: '53w',
      seconds: 53 * 604_800,
      reason: 'exceeds maximum TTL (53 weeks)',
    },
    {
      input: '8761h',
      seconds: 8761 * 3600,
      reason: 'exceeds maximum TTL (8761 hours)',
    },
  ];
