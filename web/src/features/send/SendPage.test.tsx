...
const fakeEnvelope = { v: 1 as const, suite: 'v1-pbkdf2-hkdf-aes256gcm-sealed-payload' as const, enc: { alg: 'A256GCM' as const, nonce: 'n', ciphertext: 'c' }, kdf: { name: 'none' as const }, hkdf: { hash: 'SHA-256' as const, salt: 's', enc_info: 'e', claim_info: 'cl', length: 32 as const } };
...
describe('SendPage', () => {
  beforeEach(() => {
    mockFetchInfo.mockResolvedValue({
      ...
      limits: {
        public: { max_envelope_bytes: 262144, max_secrets: 100, max_total_bytes: 10485760, rate: { requests_per_second: 2, burst: 5 } },
        authed: { max_envelope_bytes: 1048576, max_secrets: 1000, max_total_bytes: 104857600, rate: { requests_per_second: 10, burst: 20 } },
      },
    ...
    mockCreate.mockResolvedValue({ id: 'sec_abc', expires_at: '2026-03-01T00:00:00Z' });
  ...
  it('enables submit when text is entered', async () => {
    ...
    render(<SendPage />);
    await user.type(screen.getByPlaceholderText('Enter your secret...'), 'hello');
  ...
  it('successful text submission shows ShareResult', async () => {
    const user = userEvent.setup();
    render(<SendPage />);
    await user.type(screen.getByPlaceholderText('Enter your secret...'), 'my secret');
  ...
  it('passes passphrase to seal when provided', async () => {
    const user = userEvent.setup();
    render(<SendPage />);
    await user.type(screen.getByPlaceholderText('Enter your secret...'), 'text');
  ...
  it('shows error when envelope too large', async () => {
    mockCheckSize.mockReturnValue('Secret is too large. Maximum size is 256 KB.');
    const user = userEvent.setup();
    render(<SendPage />);
    await user.type(screen.getByPlaceholderText('Enter your secret...'), 'text');
    await user.click(screen.getByRole('button', { name: 'Create secret' }));
  ...
  it('shows button text transitions during submission', async () => {
    ...
    mockSeal.mockImplementation(() => new Promise((r) => { resolveSeal = r; }));
    ...
    await waitFor(() => {
      expect(screen.getByRole('button', { name: /Encrypting/ })).toBeInTheDocument();
    });
