...
// --- Web-to-web pairing flow types ---
//
// Distinct from `device-auth` (CLI) and `app-login` (Tauri) — both of those
// flows mint an API key on approval. Web-to-web pairing only transfers the
// AMK; no API key is created, no `auth_token` is involved. See
// `spec/v1/server.md` "Web-to-web AMK transfer".
...
struct PairStartResponse {
    user_code: String,
    /// Private bearer token. The displayer uses this on `/poll` and `/cancel`.
    /// MUST NOT be embedded in QR codes or URLs.
...
/// joinable. Lets the joiner UI render specific copy instead of a generic
/// error. State is one of: 'claimed' | 'approved' | 'cancelled'.
#[derive(Serialize)]
struct PairChallengeTerminalState {
    state: String,
}
...
struct PairApproveRequest {
    user_code: String,
    amk_transfer: AmkTransferJson,
}

#[derive(Deserialize)]
#[serde(deny_unknown_fields)]
struct PairCancelRequest {
    poll_token: String,
}
...
pub fn build_router(state: Arc<AppState>) -> Router {
    ...
    router
        ...
        .route("/api/v1/auth/pair/approve", any(handle_pair_approve_entry))
        .route("/api/v1/auth/pair/cancel", any(handle_pair_cancel_entry))
...
// --- Web-to-web pairing flow handlers (purpose=web-pair) ---
//
// Both displayer and joiner sides require session auth. The slot's user_id
// is set at /start; every subsequent endpoint cross-checks
...
pub async fn handle_pair_start_entry(State(state): State<Arc<AppState>>, req: Request) -> Response {
    if req.method() != Method::POST {
        return method_not_allowed();
    }
    ...
    let payload: PairStartRequest = match read_json_body(req, 4 * 1024, None).await {
        Ok(v) => v,
        Err(resp) => return resp,
    };
    ...
    let displayer_poll_token = match random_b64(PAIR_POLL_TOKEN_LEN) {
        Ok(v) => v,
        Err(_) => return internal_server_error(),
    };
    ...
    let now = Utc::now();
    let expires_at = now + chrono::Duration::seconds(WEB_PAIR_EXPIRY_SECS);
    if let Err(e) = state
        .auth_store
        .insert_challenge(
            // The displayer's poll token IS the row's challenge_id — primary
            // key lookup serves the displayer's /poll directly.
            &displayer_poll_token,
            // Bind slot to the calling session's user. Every subsequent
            // endpoint enforces same-user via require_session_user + this
            // user_id field.
            Some(user_id),
            WEB_PAIR_PURPOSE,
            &challenge_json,
        ...
        .await
    {
        error!(error = %e, "failed to insert web-pair challenge");
        return internal_server_error();
    }
...
pub async fn handle_pair_poll_entry(State(state): State<Arc<AppState>>, req: Request) -> Response {
    if req.method() != Method::POST {
        return method_not_allowed();
    }
    ...
    let payload: PairPollRequest = match read_json_body(req, 1024, None).await {
        Ok(v) => v,
        Err(resp) => return resp,
    };
    ...
    let data: PairChallengeJson = match serde_json::from_str(&record.challenge_json) {
        Ok(v) => v,
        Err(_) => return internal_server_error(),
    };
...
pub async fn handle_pair_challenge_entry(
    State(state): State<Arc<AppState>>,
    query: Query<PairChallengeQuery>,
    req: Request,
) -> Response {
    if req.method() != Method::GET {
        return method_not_allowed();
    ...
    let record = match state
        .auth_store
        .find_challenge_by_user_code(&query.user_code, WEB_PAIR_PURPOSE, now)
        .await
    {
        Ok(c) => c,
        Err(StorageError::NotFound) => {
        ...
        Err(e) => {
            error!(error = %e, "failed to find web-pair slot for challenge");
            return internal_server_error();
        }
    };
    ...
    // Distinguishable terminal-state response — the joiner UI renders
    // specific copy ("already in use" / "already approved" / "cancelled").
    if data.status != "pending" {
        return json_response(
            StatusCode::CONFLICT,
            PairChallengeTerminalState { state: data.status },
        );
    }
...
pub async fn handle_pair_approve_entry(
    State(state): State<Arc<AppState>>,
    req: Request,
) -> Response {
    if req.method() != Method::POST {
    ...
    let payload: PairApproveRequest = match read_json_body(req, 4 * 1024, None).await {
        Ok(v) => v,
        Err(resp) => return resp,
    };

    if let Err(msg) = validate_amk_transfer(&payload.amk_transfer) {
        return bad_request(msg);
    }
        ...
        .await
    {
        ...
        Err(e) => {
            error!(error = %e, "failed to find web-pair slot for approve");
            return internal_server_error();
        }
    };
    ...
    data.amk_transfer = Some(payload.amk_transfer);
        ...
        .await
    {
        ...
        Err(StorageError::NotFound) => {
            return error_response(
                StatusCode::CONFLICT,
                "slot transitioned out from under approve",
            );
        }
        Err(e) => {
            error!(error = %e, "failed to CAS-update web-pair slot for approve");
            return internal_server_error();
        }
    }
...
pub async fn handle_pair_cancel_entry(
    State(state): State<Arc<AppState>>,
    req: Request,
) -> Response {
    if req.method() != Method::POST {
    ...
    let payload: PairCancelRequest = match read_json_body(req, 1024, None).await {
        Ok(v) => v,
        Err(resp) => return resp,
    };
        ...
        .await
    {
        Ok(()) => {}
        Err(StorageError::NotFound) => {
            // Concurrent transition won — also fine, idempotent terminal.
            return json_response(StatusCode::OK, serde_json::json!({ "ok": true }));
        }
        Err(e) => {
            error!(error = %e, "failed to CAS-update web-pair slot for cancel");
            return internal_server_error();
        }
    }
    ...
    impl AuthStore for MemStore {
        ...
        async fn cas_update_challenge_json(
            ...
            let rec = m
                ...
                .filter(|r| r.purpose == purpose && r.expires_at > now)
                .ok_or(StorageError::NotFound)?;
            let current_status = serde_json::from_str::<serde_json::Value>(&rec.challenge_json)
                .ok()
                .and_then(|v| v.get("status").and_then(|s| s.as_str()).map(String::from))
                .unwrap_or_default();
            if !expected_statuses.iter().any(|s| *s == current_status) {
                return Err(StorageError::NotFound);
            }
